...
const SubMonthlyplan = ({navigation}: any) => {
  ...
  const renderItem = ({item, index}) => (
    <TouchableOpacity onPress={() => toggleImageVisibility(index)}>
      <View
        style={[
          styles.card,
          {borderColor: item.showImage ? Colors.Green : Colors.light_green},
        ]}>
        <View style={styles.main_card_view}>
          <ViewCommon>
            <TouchableOpacity style={styles.selecticon}>
              {item.showImage && (
                <Image source={imgUrl.Ellipse} style={styles.selectedimg} />
              )}
            </TouchableOpacity>
            <CommonText style={[fontstylee.eighteen_px_txt, {marginLeft: 10}]}>
              {item.miles}
            </CommonText>
          </ViewCommon>
          <ViewCommon>
    ...
    <SafeView style={{padding: 0}}>
      ...
      <ScrollView style={styles.scrollview_style}>
        <View style={styles.texts}>
          ...
          <CommonText style={{marginTop: 7}}>
            {Texts.Insurance_included}
          </CommonText>
        </View>
        <FlatList
          data={data}
